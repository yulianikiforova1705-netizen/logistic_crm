...
const multer = require('multer'); // НОВОЕ: библиотека для загрузки файлов
const fs = require('fs');         // НОВОЕ: работа с файловой системой

const app = express();
const PORT = 3000;

// ТВОЯ ССЫЛКА ДЛЯ ПОДКЛЮЧЕНИЯ ИЗ NEON (вставь её сюда!)
...
// НОВОЕ: Настройка сохранения файлов (уникальные имена, чтобы не было конфликтов)
...
app.use('/uploads', express.static(uploadDir)); // Разрешаем браузеру скачивать файлы из этой папки
...
app.post('/api/orders', async (req, res) => {
    ...
    try {
        const result = await pool.query('INSERT INTO orders (order_date, client_name, contractor_name, route, client_rate, contractor_rate) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [orderDate, clientName, contractorName, route, clientRate, contractorRate]);
...
// Добавление текстовой записи (оставили без изменений)
app.post('/api/orders/:id/history', async (req, res) => {
    try {
        await pool.query("UPDATE orders SET history = COALESCE(history, '[]'::jsonb) || $1::jsonb WHERE id = $2", [JSON.stringify([{ date: new Date().toISOString(), text: req.body.text }]), req.params.id]);
        res.json({ status: "success" });
    } catch (err) { res.status(500).json({ status: "error", message: err.message }); }
});

// НОВОЕ: Загрузка файла в историю рейса
app.post('/api/orders/:id/history_file', upload.single('document'), async (req, res) => {
    try {
        if (!req.file) throw new Error("Файл не найден");
        const fileUrl = '/uploads/' + req.file.filename;
        const text = `📎 Прикреплен документ: ${req.file.originalname}`;
        
        // Добавляем запись в историю, но теперь с ключом fileUrl
        await pool.query(
            "UPDATE orders SET history = COALESCE(history, '[]'::jsonb) || $1::jsonb WHERE id = $2",
            [JSON.stringify([{ date: new Date().toISOString(), text: text, fileUrl: fileUrl }]), req.params.id]
        );
